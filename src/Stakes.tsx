...
const StakesList = (props: {
  ...
  const stakeListOutput = stakeList.map((stakeData) => {
    ...
    const { stakedHearts, stakeShares, payout, bigPayDay, penalty } = stakeData;
    ...
    totalUsdValue +=
      Number(formatUnits(stakedHearts + payout + bigPayDay, HEX.DECIMALS)) *
      props.usdhex;
      ...
      <Card className="mt-1 bg-info-darkened rounded">
        <Card.Body className="p-1 rounded text-light">
          ...
          <Row>
            ...
            <Col>
              <CryptoVal value={stakedTotal} showUnit />
              <span className="text-muted small"> {t("BURNED")}</span>
            </Col>
          </Row>
          <Row>
            <Col className="text-end">{t("Shares")}</Col>
            <Col>
              <CryptoVal value={sharesTotal} currency="SHARES" />
            </Col>
          </Row>
          <Row>
            <Col className="text-end text-bold">{t("Yield")}</Col>
            <Col>
              <CryptoVal value={payoutTotal} showUnit />
            </Col>
          </Row>
          {bigPayDayTotal > 0n && (
            <Row>
              ...
              <Col>
                <CryptoVal value={bigPayDayTotal} showUnit />
              </Col>
            </Row>
          )}
          <Row>
            ...
            <Col>
              <CryptoVal
                className="text-bold"
                value={stakedTotal + payoutTotal + bigPayDayTotal}
                showUnit
              />
            </Col>
          </Row>
          <Row className="text-success">
            <Col className="text-success text-end text-bold">
              {t("USD Value")}
            </Col>
            <Col className="text-success text-bold">
              $<CryptoVal value={totalUsdValue} currency="USD" />
            </Col>
          </Row>
          <Row className="mt-2">
            <Col className="text-end text-bold">{t("Mean Net Yield")}</Col>
            <Col>
              <CryptoVal value={averagePercentGain} currency="%" />%
            </Col>
          </Row>
          <Row>
            <Col className="text-end text-bold">{t("Average APY")}</Col>
            <Col>
              <CryptoVal value={averagePercentAPY} currency="%" />%
            </Col>
          </Row>
        </Card.Body>
      </Card>
      <Card className="active-stakes m-0 text-light bg-success-darkened rounded">
        <Card.Body className="px-2">
          <h2 className="text-center mb-0">
            {numStakes ? <span className="numeric">{numStakes}</span> : "No"}{" "}
            {t("Active ")}
            {numStakes > 1 ? t("Stakes") : t("Stake")}
          </h2>
          <div className="text-center text-info small">
            {t("tap each for details")}
          </div>
          {!!hexData &&
            stakeListOutput?.map((stake) => {
              return (
                <StakeInfo
...
const Stakes = (props: {
  ...
  const [selectedCard, setSelectedCard] = useState(
    props.openActive ? "current_stakes" : ""
  );
  ...
  useContractReads({
    ...
    onSuccess: (data) => {
      ...
      const _stakeList = data.map((stake, index: number): StakeData => {
        ...
        const progress =
          ((Number(currentDay) - Number(lockedDay)) / Number(stakedDays)) * 100;
        ...
        const partDayPayout =
          currentDay >= lockedDay && currentDay < lockedDay + stakedDays
  ...
  /**
   * Daily Data
   * To calculate yield, we meed to load daily data for each day our stakes were active.
   * Rather than doing that for each stake, we load all daily data just once, from the
   * earliest day from our stakes list, up to the latests day, which should be currentDay-1
   * BUT sometimes isn't, when the when the calendar day clocks over midnight UTC but
   * the contract's dailyDataUpdate() hasn't been run yet.
   */
  const dailyDataCount =
    hexData?.globals?.dailyDataCount || hexData?.currentDay || 0n;
  ...
  return (
    <>
      ...
      >
        ...
        <Accordion.Item
          className={"text-light " + (props.className || "")}
          eventKey="current_stakes"
        >
          {props.account && (
            <div className="bg-secondary px-1 text-light text-center small">
              <span className="text-info small align-baseline me-2">
                {accountName}
              </span>
              {walletAddress}
            </div>
          ...
          <Accordion.Header className="w-100">
            <Row className="w-100">
              <Col className="pe-0">
                <BurgerHeading>{t("Active Stakes")}</BurgerHeading>
              </Col>
              <Col className="col-5 lh-lg px-0 text-end text-success">
                <small className="text-muted small align-baseline me-1">
                  USD ATP
                </small>
                <CryptoVal
                  className="fw-bold"
